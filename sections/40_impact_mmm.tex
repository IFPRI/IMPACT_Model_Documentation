...
\begin{equation}
    \label{eq:landsupply}
    QFS_{fpu, lnd} = QFSInt_{fpu, lnd} * QFSInt2_{fpu, lnd} * QFSInt3_{fpu, lnd} * \left (WF_{fpu, lnd}\right )^{L_{\gamma}} 
\end{equation}
where:
\begin{conditions}
 QFS        &   Land supply \\
 QFSInt     &   Land supply intercept i.e., base year supply of land \\   
 QFSInt2    &   Endogenous land supply growth multiplier \\   
 QFSInt3    &   Exogenous land supply growth multiplier \\   
...
Additionally, equations \ref{eq:landsupply2a} and \ref{eq:landsupply2b} are applied at the end of each year before solving for land supply for a new year. Note that the index \textit{t} and \textit{t+1} for the simulation time step are only illustrative in equations \ref{eq:landsupply2a} and \ref{eq:landsupply2b} to denote updates of parameters in IMPACT from one year to another.
\begin{subequations}
    \begin{equation}
        \label{eq:landsupply2a}
        QFSInt2_{fpu, lnd, t+1} = QFSInt2_{fpu, lnd, t} * (1 + Landgr_{fpu, lnd}) 
    \end{equation}
    \begin{equation}
       \label{eq:landsupply2b}
       QFSInt3_{fpu, lnd, t+1} =  \left ( \frac{\sum_{t-3}^{t} WF_{fpu, lnd, t}}{3} \right )^{L_{\gamma, t}} 
    \end{equation}
\end{subequations}
...
\begin{figure}[hbt!]
  ...
  MMJ is \textbf{M}arketing \textbf{M}argins for activities (\textbf{j}).\\
  PSE is \textbf{P}roducer \textbf{S}ubsidy \textbf{E}quivalents.\\
  CSE is \textbf{C}onsumer \textbf{S}ubsidy \textbf{E}quivalents. \\
  MMM is \textbf{M}arketing \textbf{M}argins for I\textbf{m}ports \\ 
  MME is \textbf{M}arketing \textbf{M}argins for \textbf{E}xports \\
  TM is \textbf{T}ariffs on \textbf{I}mports, and,\\
  TE is \textbf{T}axes on \textbf{E}xports.}
  \label{fig:price_str}
\end{figure}
